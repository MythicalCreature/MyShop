...
<script type="text/javascript">
    function pay(oid) {
        if(confirm("确认是否支付")){
            location.href="orders?method=success&oid="+oid+"&result=SUCCESS";
        }else{
            location.href="orders?method=success&oid="+oid+"&result=ERROR";
        }
    }
</script>
</head>
		...
		<tr>
			<td align="right" colspan="4" style="margin-right: 40px;">
				<c:if test="${orders.ostate eq 1 }">
					<button type="button" onclick="pay('${orders.oid}')" class="btn btn-success btn-sm">立刻支付</button>
				</c:if>
			</td>
		</tr>
	</table>
