...
<script type="text/javascript">
	...
	function getOrder(orderId){
		location.href="${pageContext.request.contextPath}/orders?method=getOrder&oid="+orderId;
	}
</script>
</head>
			...
			<c:forEach items="${ordersList}" var="order" varStatus="i">
				<tr>
					...
					<th>
						...
						<c:if test="${order.ostate eq 3 }">
							<button type="button" class="btn btn-warning btn-sm" onclick="getOrder('${order.oid}')">确认收货</button>
						</c:if>
					</th>
				</tr>
			</c:forEach>
